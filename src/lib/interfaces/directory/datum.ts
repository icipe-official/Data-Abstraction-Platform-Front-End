...
export async function Datum(
	...
	let d: Domain.Interfaces.Directory.Datum = {
		...
		async create(
			...
			let sectionName = opts?.componentName || `${Domain.Entities.Url.Action.CREATE}  ${Domain.Entities.Directory.RepositoryName}`
		...
		async update(
			...
			let sectionName = opts?.componentName || `${Domain.Entities.Url.Action.UPDATE} ${Domain.Entities.Directory.RepositoryName}`
		...
		async delete(
			...
			let sectionName = opts?.componentName || `${Domain.Entities.Url.Action.DELETE} ${Domain.Entities.Directory.RepositoryName}`
		...
		if (directoryGroupID) {
			try {
				d.dataMetadataModel = await new Interfaces.MetadataModels.FieldAnyGetMetadataModel(telemetry, authFetch).GetMetadataModel(
					...
					fetchedData?.metadata_model[MetadataModel.FgProperties.DATABASE_TABLE_COLLECTION_UID] || Domain.Entities.Directory.RepositoryName,
					[directoryGroupID]
				)
			} catch (e) {
				telemetry?.Log(
