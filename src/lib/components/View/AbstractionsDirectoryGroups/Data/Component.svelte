<script lang="ts">
	...
	const COMPONENT_NAME = 'view-abstractions-directory-groups-data'
						...
						switch (field[MetadataModel.FgProperties.DATABASE_TABLE_COLLECTION_NAME]) {
							case Domain.Entities.AbstractionsDirectoryGroups.RepositoryName:
								switch (field[MetadataModel.FgProperties.DATABASE_FIELD_COLUMN_NAME]) {
									case Domain.Entities.AbstractionsDirectoryGroups.FieldColumn.Description:
										ntc += 1
										break
								}
								break
							...
							{#await import('../Datum/Component.svelte') then { default: Datum }}
								<main class="z-[1] p-2">
									<Datum
										...
										title={'Group Abstraction Setup Information'}
									></Datum>
								</main>
							{/await}
		...
		{:else}
			<main class="relative z-[1] grid overflow-auto" style="grid-template-columns: subgrid; grid-column: span {gridTemplateColumns};">
				...
				>
					...
					{#if getFieldGroupByFieldColumnName(Domain.Entities.AbstractionsDirectoryGroups.RepositoryName, Domain.Entities.AbstractionsDirectoryGroups.FieldColumn.Description, 0)}
						{@render headerFieldColumn('Abstraction Description/Purpose')}
					{/if}
				</header>
		...
		>
			{#if containerWidth > 375 && noOfRows > tablerowcontentperpage}
				<span class="sticky flex w-fit gap-x-2" style="left: 0px;">
					<span class="input">
						<span class="label">Total No of Groups with Abstractions setup:</span>
						<span>{noOfRows}</span>
					</span>
				</span>
			{/if}
