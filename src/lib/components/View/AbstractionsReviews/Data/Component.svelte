<script lang="ts">
	...
	const COMPONENT_NAME = 'view-abstractions-reviews-data'
				...
				switch (field[MetadataModel.FgProperties.DATABASE_JOIN_DEPTH]) {
					case 0:
						switch (field[MetadataModel.FgProperties.DATABASE_TABLE_COLLECTION_NAME]) {
							case Domain.Entities.AbstractionsReviews.RepositoryName:
								switch (field[MetadataModel.FgProperties.DATABASE_FIELD_COLUMN_NAME]) {
									case Domain.Entities.AbstractionsReviews.FieldColumn.AbstractionsID:
										ntc += 1
										break
									case Domain.Entities.AbstractionsReviews.FieldColumn.DirectoryID:
										ntc += 1
										break
									case Domain.Entities.AbstractionsReviews.FieldColumn.ReviewPass:
										ntc += 1
										break
									case Domain.Entities.AbstractionsReviews.FieldColumn.LastUpdatedOn:
										ntc += 1
										break
								}
						}
							...
							{#await import('../Datum/Component.svelte') then { default: Datum }}
								<main class="z-[1] p-2">
									<Datum
										...
										title={'Abstraction Check Information'}
									></Datum>
								</main>
							{/await}
		...
		{:else}
			<main class="relative z-[1] grid overflow-auto p-2" style="grid-template-columns: subgrid; grid-column: span {gridTemplateColumns};">
				...
				>
					...
					{#if getFieldGroupByFieldColumnName(Domain.Entities.AbstractionsReviews.RepositoryName, Domain.Entities.AbstractionsReviews.FieldColumn.AbstractionsID, 0)}
						{@render headerFieldColumn('Abstraction ID')}
					{/if}

					{#if getFieldGroupByFieldColumnName(Domain.Entities.AbstractionsReviews.RepositoryName, Domain.Entities.AbstractionsReviews.FieldColumn.DirectoryID, 0)}
						{@render headerFieldColumn('Reviewer Directory ID')}
					{/if}
					...
					{#if getFieldGroupByFieldColumnName(Domain.Entities.Directory.RepositoryName, Domain.Entities.Directory.FieldColumn.DisplayName, 1)}
						{@render headerFieldColumn('Reviewer Directory Name')}
					{/if}

					{#if getFieldGroupByFieldColumnName(Domain.Entities.AbstractionsReviews.RepositoryName, Domain.Entities.AbstractionsReviews.FieldColumn.ReviewPass, 0)}
						{@render headerFieldColumn('Check Passed?')}
					{/if}

					{#if getFieldGroupByFieldColumnName(Domain.Entities.AbstractionsReviews.RepositoryName, Domain.Entities.AbstractionsReviews.FieldColumn.LastUpdatedOn, 0)}
						{@render headerFieldColumn('Last Updated On')}
					{/if}
				</header>
				...
				<main class="z-[1] grid" style="grid-template-columns: subgrid; grid-column: span {gridTemplateColumns};">
					{#each Utils.Range(rowsStart, Utils.RangeArrayEnd(rowsEnd, noOfRows)) as rIndex (rIndex)}
						...
						>
							...
							{#if getFieldGroupByFieldColumnName(Domain.Entities.AbstractionsReviews.RepositoryName, Domain.Entities.AbstractionsReviews.FieldColumn.LastUpdatedOn, 0)}
								{@render datumlastupdatedon(rowIndex)}
							{/if}
						</section>
					{/each}
				</main>
...
{#snippet datumabstractionid(dIndex: number, joinDepth: number = 0)}
	{@const fieldData = getDatumFieldData(
		dIndex,
		Domain.Entities.AbstractionsReviews.RepositoryName,
...
{#snippet datumdirectoryid(dIndex: number, joinDepth: number = 0)}
	{@const fieldData = getDatumFieldData(
		dIndex,
		Domain.Entities.AbstractionsReviews.RepositoryName,
		Domain.Entities.AbstractionsReviews.FieldColumn.DirectoryID,
		joinDepth
	)}
...
{#snippet datumreviewpass(dIndex: number, joinDepth: number = 0)}
	{@const fieldData = getDatumFieldData(
		dIndex,
		Domain.Entities.AbstractionsReviews.RepositoryName,
		Domain.Entities.AbstractionsReviews.FieldColumn.ReviewPass,
		joinDepth
	)}
...
{#snippet datumlastupdatedon(dIndex: number, joinDepth: number = 0)}
	{@const fieldData = getDatumFieldData(
		dIndex,
		Domain.Entities.AbstractionsReviews.RepositoryName,
		Domain.Entities.AbstractionsReviews.FieldColumn.LastUpdatedOn,
		joinDepth
	)}
		...
		<div
			class="btn-circle btn-lg flex justify-center self-center {theme === Domain.Entities.Theme.Theme.LIGHT
				...
				: switchbackground
					...
					: 'border-gray-400 bg-gray-200'}"
		>
			<!--mdi:notebook-check source: https://icon-sets.iconify.design-->
			<svg class="self-center" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
				<path
					...
					d="m16.75 22.16l-2.75-3L15.16 18l1.59 1.59L20.34 16l1.16 1.41zM3 7V5h2V4a2 2 0 0 1 2-2h6v7l2.5-1.5L18 9V2h1c1.05 0 2 .95 2 2v9.8a6.005 6.005 0 0 0-8.2 8.2H7c-1.05 0-2-.95-2-2v-1H3v-2h2v-4H3v-2h2V7zm2-2v2h2V5zm0 14h2v-2H5zm0-6h2v-2H5z"
				/>
			</svg>
		</div>

		<div class="flex flex-col gap-y-1">
			<span>{@render datumabstractionid(dIndex)}</span>
			<span><span class="font-bold italic">Check Passed?</span> {@render datumreviewpass(dIndex)}</span>
		</div>
	</div>
{/snippet}
