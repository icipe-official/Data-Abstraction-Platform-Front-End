<script lang="ts">
	...
	let metadataModelsDirectorySearch: Domain.Interfaces.MetadataModels.Search | undefined = $derived.by(() => {
		if (
	...
	let metadataModelsDirectoryQueryConditions: MetadataModel.QueryConditions[] = $state([])
	let quickSearchQueryCondition: MetadataModel.QueryConditions = $state({})
	let metadataModelsDirectorySearchMetadataModel: any = $state({})
	let metadataModelsDirectorySearchResults: any[] = $state([])
	let metadataModelsDirectorySearchFilterExcludeIndexes: number[] = $state([])

	async function getDisplayMetadataModelsDirectory() {
		if (!metadataModelsDirectorySearch) {
			throw [401, 'Unauthorized']
		}

		if (Object.keys(metadataModelsDirectorySearch.searchmetadatamodel).length === 0) {
			try {
				await metadataModelsDirectorySearch.FetchMetadataModel(authContextDirectoryGroupID, 1, undefined)
			} catch (e) {
		...
		metadataModelsDirectorySearch.searchmetadatamodel = MetadataModel.MapFieldGroups(
			metadataModelsDirectorySearch.searchmetadatamodel,
			(property: any) => {
				if (
		...
		metadataModelsDirectorySearchMetadataModel = metadataModelsDirectorySearch.searchmetadatamodel
		try {
			await searchMetadataModelsDirectory()
		} catch (e) {
			throw e
		}
	}
	...
	function updateMetadataModel(value: any) {
		metadataModelsDirectorySearchMetadataModel = value
		if (metadataModelsDirectorySearch) {
			metadataModelsDirectorySearch.searchmetadatamodel = metadataModelsDirectorySearchMetadataModel
		}
	}

	async function searchMetadataModelsDirectory() {
		if (!metadataModelsDirectorySearch) {
			return
		}
		...
		try {
			await metadataModelsDirectorySearch.Search(
				Utils.MetadataModel.InsertNewQueryConditionToQueryConditions(metadataModelsDirectoryQueryConditions, [quickSearchQueryCondition]),
			...
			metadataModelsDirectorySearchFilterExcludeIndexes = []
			metadataModelsDirectorySearchResults = metadataModelsDirectorySearch.searchresults.data || []

			State.Toast.Type = Domain.Entities.Toast.Type.INFO
			State.Toast.Message = `${metadataModelsDirectorySearchResults.length} results returned`
		} catch (e) {
	...
	let showMetadataModelsDirectoryQueryPanel: boolean = $state(false)

	let windowWidth: number = $state(0)

	let selectedMetadataModelsDirectory: number[] = $state([])
	...
	async function deleteMetadataModelsDirectory() {
		const data = selectedMetadataModelsDirectory.map((dIndex) => metadataModelsDirectorySearchResults[dIndex])
		...
		try {
			...
			if (fetchResponse.ok) {
				...
				selectedMetadataModelsDirectory = []
	...
	let createMetadataModelsDirectoryDataValid: boolean = $derived(selectedMetadataModels.length > 0 && selectedDirectoryGroups.length > 0)
	...
	let updateMetadataModelsDirectoryDataValid: boolean = $derived(selectedMetadataModels.length > 0 && selectedMetadataModelsDirectory.length > 0)
	async function handleUpdateMetadataModelsDirectory() {
		...
		let newMetadataModelsDirectory: Domain.Entities.MetadataModelsDirectory.Interface[] = selectedMetadataModelsDirectory.map((smmdIndex) => {
			let d: Domain.Entities.MetadataModelsDirectory.Interface = metadataModelsDirectorySearchResults[smmdIndex]
...
<div class="flex flex-1 flex-col gap-y-2 overflow-hidden">
	{#await getDisplayMetadataModelsDirectory()}
	...
	{:then}
		...
		{:else}
				...
				{#await import('$lib/components/View/MetadataModelsDirectory/SearchBar/Component.svelte') then { default: ViewMetadataModelsDirectorySearchBar }}
					<div class="max-md:w-full md:w-[60%]">
						<ViewMetadataModelsDirectorySearchBar
							metadatamodel={metadataModelsDirectorySearchMetadataModel}
							{themecolor}
							{theme}
							{telemetry}
							...
							showquerypanel={() => {
								showMetadataModelsDirectoryQueryPanel = !showMetadataModelsDirectoryQueryPanel
							}}
							search={() => {
								searchMetadataModelsDirectory()
							}}
						></ViewMetadataModelsDirectorySearchBar>
					</div>
				{/await}
			...
			<main class="z-[1] flex flex-[9.5] gap-x-2 overflow-hidden">
				{#if showMetadataModelsDirectoryQueryPanel}
					<section class="flex flex-[2] flex-col gap-y-2 overflow-hidden">
						{#await import("$lib/components/QueryPanel/Component.svelte") then { default: QueryPanel }}
							<QueryPanel
								{themecolor}
								...
								{telemetry}
								metadatamodel={metadataModelsDirectorySearchMetadataModel}
								data={metadataModelsDirectorySearchResults}
								queryconditions={metadataModelsDirectoryQueryConditions}
								filterexcludeindexes={metadataModelsDirectorySearchFilterExcludeIndexes}
								updatefilterexcludeindexes={(value) => {
									metadataModelsDirectorySearchFilterExcludeIndexes = value
									State.Toast.Type = Domain.Entities.Toast.Type.INFO
									State.Toast.Message = `${metadataModelsDirectorySearchFilterExcludeIndexes.length} local results filtered out`
								}}
								updatemetadatamodel={updateMetadataModel}
								updatequeryconditions={(value) => {
									metadataModelsDirectoryQueryConditions = value
								}}
								hidequerypanel={() => (showMetadataModelsDirectoryQueryPanel = false)}
							></QueryPanel>
						{/await}
					</section>
				{/if}

				{#if !showMetadataModelsDirectoryQueryPanel || windowWidth > 1000}
					{#if metadataModelsDirectorySearchResults.length > 0}
						<section class="flex {windowWidth > 1500 ? 'flex-[3]' : 'flex-2'} flex-col overflow-hidden rounded-lg">
							<section class="z-[2] flex w-full">
								{#if selectedMetadataModelsDirectory.length > 0}
									<div class="flex flex-col p-2">
										...
										>
											...
											<span class="self-center">{selectedMetadataModelsDirectory.length} selected</span>
										</button>
							...
							<section class="z-[1] flex h-full w-full flex-1 flex-col overflow-hidden">
								{#await import('$lib/components/View/MetadataModelsDirectory/Data/Component.svelte') then { default: ViewMetadataModelsDirectoryData }}
									<ViewMetadataModelsDirectoryData
										...
										addclickcolumn={false}
										addselectcolumn={true}
										selecteddataindexes={selectedMetadataModelsDirectory}
										updateselecteddataindexes={(value) => (selectedMetadataModelsDirectory = value)}
										view={dataView}
										updatemetadatamodel={updateMetadataModel}
										filterexcludeindexes={metadataModelsDirectorySearchFilterExcludeIndexes}
									></ViewMetadataModelsDirectoryData>
